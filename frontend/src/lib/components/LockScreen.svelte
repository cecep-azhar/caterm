<script lang="ts">
  ...
  async function handleUnlock() {
    if (!password) {
      errorMsg = 'Please enter your master password.';
      return;
    }
    isLoading = true;
    errorMsg = '';
    try {
      // Call Tauri backend validate_vault_password
      await invoke('validate_vault_password', { password });
      onUnlocked();
    } catch (err: any) {
      errorMsg = typeof err === 'string' ? err : (err?.message || 'Invalid Master Password (min 8 characters)');
    } finally {
      isLoading = false;
    }
  }
...
<div class="fixed inset-0 z-50 flex bg-[#0a0a0a] text-white select-none">
  ...
  <div class="hidden lg:flex flex-1 flex-col justify-between p-12 bg-neutral-950 border-r border-neutral-800/60 relative overflow-hidden">
    ...
    <div class="relative z-10 flex items-center gap-3">
      <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-500 to-indigo-600 flex items-center justify-center font-bold text-xl shadow-lg shadow-sky-500/20">
        C
      </div>
      <div>
        <h1 class="text-xl font-bold tracking-wider text-white">CATerm <span class="text-xs px-2 py-0.5 rounded bg-sky-500/20 text-sky-400 border border-sky-500/30">v2.0.5</span></h1>
    ...
    <div class="relative z-10 my-auto max-w-lg">
      ...
      <div class="mt-8 p-4 rounded-xl bg-neutral-900/80 border border-neutral-800 text-xs text-neutral-400 space-y-2">
        <div class="flex items-center gap-2 text-emerald-400 font-medium">
          ...
          Zero-Knowledge Local Encryption (SQLCipher)
        </div>
    ...
    <div class="relative z-10 text-xs text-neutral-500">
      Dedicated to <span class="text-neutral-300 font-medium">Umar Abdillah</span> by <span class="text-neutral-300 font-medium">Cecep Azhar</span>
    </div>
  </div>
  ...
  <div class="flex-1 flex flex-col justify-center items-center p-8 sm:p-16 bg-[#0a0a0a]">
    <div class="w-full max-w-md space-y-8">
      <div class="text-center">
        ...
        <h2 class="text-2xl font-bold text-white">Unlock CATerm Vault</h2>
        <p class="text-sm text-neutral-400 mt-1">Logged in as <span class="text-sky-400 font-mono">cecep.azhtech@gmail.com</span></p>
      </div>
      ...
      <div class="space-y-4">
        <div>
          <label for="master-password" class="block text-xs font-semibold uppercase tracking-wider text-neutral-400 mb-2">Master Password</label>
          <input
            id="master-password"
            type="password"
            bind:value={password}
      ...
      <div class="flex items-center justify-between text-xs text-neutral-500 pt-4 border-t border-neutral-900">
        <button onclick={() => alert('Vault reset will clear local key. Re-enter master password.')} class="hover:text-neutral-400 transition-colors">
          Reset Vault
        </button>
